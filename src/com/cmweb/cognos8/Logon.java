 ...
 *
 * Description:  This code sample demonstrates how to log on to IBM Cognos 8 
 *               and how to log off using the following IBM Cognos 8 SDK methods:
 *
 *             - logon
...
public class Logon {
	...
	/**
	 * Use this Java method to log on to the IBM Cognos 8 system, bypassing any
	 * prompts.
	 * 
	 * @param connection
	...
	/**
	 * Use this Java method to logon to the IBM Cognos 8 system.
	 * 
	 * @param connection
	 *            Connection to IBM Cognos 8
	 * 
	...
	public String logon(CRNConnect connection) {
		...
		if (namespaceInfo == null) {
			...
			return "Unable to connect to IBM Cognos 8";
		}
	...
	public static boolean doTestForAnonymous(CRNConnect connection) {
		...
		try {
			BaseClass bc[] = connection.getCMService().query(
					new SearchPathMultipleObject("/"), new PropEnum[] {},
	...
	// throws Exception
	{
		...
		try {
			connection.getCMService().query(new SearchPathMultipleObject("/"),
		...
		BiBusHeader bibus = (BiBusHeader) ((Stub) connection.getCMService())
				.getHeaderObject("", "biBusHeader");
		...
		((Stub) connection.getCMService()).getServiceContext().clearHeaders();
