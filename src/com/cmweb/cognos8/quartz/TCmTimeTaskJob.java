...
public class TCmTimeTaskJob implements Job {
	...
	public void execute(JobExecutionContext context)
		...
		SSOAuthManager ssoAuthManager = (SSOAuthManager) context
		.getJobDetail().getJobDataMap().get("ssoAuthManager");// SSO
			...
			if (dtlList != null && dtlList.size() > 0) {
				for (TCmTimeTaskDtlVO dtlVO : dtlList) {
					...
					if (report != null) {
						...
						if (taskVO.getSendmailaddr() != null && !"".equals(taskVO.getSendmailaddr())) {

							String[] emailsTokens = taskVO.getSendmailaddr().split(";");
							if (emailsTokens.length > 0) {
								...
								if (taskVO.getSendmailorg() != null && !"".equals(taskVO.getSendmailorg().trim())) {
								...
								returnStr = cognos8Service.emailReport(connection,
										reportObject, dtlVO.getReportName(), dtlVO.getReportName(), Integer.parseInt(taskVO.getSendmailtype()), smtpList,
										null
										
										,JSONArray .fromObject(dtlVO.getParams())//报表参数
										);
							} else {
								returnStr = "邮件地址不正确";
							}
